...
Показанный на рис.\,\ref{fig:p133} способ подключения $p$--$n$"=перехода к источнику называется \emph{прямым включением} $p$--$n$"=перехода. При \emph{обратном включении} $p$--$n$"=переход подключается наоборот: $p$"=область соединяется с <<минусом>> источника, $n$"=область~--- с <<плюсом>> (в таком случае \emph{ток через переход пренебрежимо мал}). 
