...
\begin{document}
\begin{asy}
    settings.render = 4; // Включаем обратно красивое аппаратное 3D-сглаживание деталей
    settings.prc = false;
    settings.outformat = "png"; // Генерируем PNG высокого разрешения для обработки
    ...
    // Настраиваем фиксированный фон для последующего удаления
    currentlight.background = white; 
    currentlight = light(gray(0.6), specular=gray(0.2), (5,5,10));
